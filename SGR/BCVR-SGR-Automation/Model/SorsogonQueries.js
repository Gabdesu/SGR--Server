...
// =============================================================================
// SORSOGON QUERIES
// Centralized SQL query repository for Sorsogon branch controllers.
        ...
        'ST: SBS->DW': `
            ...
            'Inv. Discrepancy': `
                ...
                SELECT
                    ...
                    CAST(ISNULL(PC.P_Details, '')                         AS NVARCHAR(255)) AS [Inventory\n Group],
            ...
            'Remittance': `
                ...
                SELECT Remittance_Date AS [Remittance Date], LEFT(Order_Type, 4) AS [Order Type], Total_Net AS [Total Net], Total_Remittance AS [Total Remittance],
            ...
            'Disbursement': `
                ...
                SELECT 
                    ...
                    p.Payment_Amount    AS [Request\n Amount],
                    p.Payment_Amount    AS [Released\n Amount],
                    0                   AS [Discrepancy /\n For Deposit],
                    ''                  AS [Requestor]
...
const prdQueries = {
    ...
    'Procurement': `
        ...
        DECLARE @EndDate   DATE = '${DATE.sql.end}';
        SELECT
            OL_BookingDate AS [Encode],
            OL_Entity AS [Entity],
            DATEDIFF(DAY, OL_BookingDate, @EndDate) as [Days Lacking],
            Item_Description AS [Product],
            Item_Brand AS [Brand],
            Quantity AS [Quantity],
            Item_Packaging AS [Packaging],
    ...
    'Expired': `
        ...
        DECLARE @EndDate   DATE = '${DATE.sql.end}';
        SELECT
            CONCAT(TBL_Category_Item_File.Item_Name, ' ', TBL_Category_File.Catg_Name) AS [Item Name],
            CASE 
                WHEN LEFT(TBL_category_item_file.Item_Description, 4) = 'BCVR' THEN '-' 
                ELSE TBL_category_item_file.Item_Description 
            END AS [Item_Description],
            REPLACE(FORMAT(TBL_Category_Item_File.Item_Exp_Date, 'MM/yyyy'), '01/2040', '-') AS [Item Exp Date],
    ...
    'Lacking Served': `
        ...
        DECLARE @EndDate   DATE = '${DATE.sql.end}';
        SELECT
            MAX(LEFT(Order_type, 4))                                AS [Order type],
            MAX(TBL_Orders_Detail.Order_No)                         AS [Order No],
            MAX(Client_Name)                                        AS [Entity],
            MAX(Client_Terms)                                       AS [PO Details],
            MAX(TBL_Orders_Detail.OrderDetail_ItemNo)               AS [Item No],
            TBL_Orders_Detail.Product_Name,
            MAX(TBL_Orders_Lacking_Details.Item_Description)        AS [Lacking Description],
            MAX(TBL_Orders_Detail.Remarks)                          AS [Remarks],
            MAX(TBL_Orders_Detail.QTY)                              AS [Order Quantity],
            SUM(TBL_Orders_Lacking_Details.Quantity)                AS [Lacking Quantity],
            MAX(OL_Purchasing)                                      AS [Purchasing Assigned],
            MAX(OL_Encoder)
        FROM TBL_Orders_Detail
        INNER JOIN TBL_Orders ON TBL_Orders.Order_No = TBL_Orders_Detail.Order_No
