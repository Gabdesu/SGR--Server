...
const BRANCHES = {
    ...
    iriga: {
        ...
        dbName:     'BCVR-IBS',
        branchCode: 'IBS',
        acnFolder:  '1AFXHjwtQPU0T4fiyb5pzdc_9iZ4FM3Hp',
...
async function runSync(branchKey, controllers) {
    ...
    try {
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (type === 'acn') {
                await ctrl.run(pool, config.acnFolder, config.branchCode, config.dbName);
            } else if (type === 'sls') {
        ...
        console.log(`${'═'.repeat(60)}`);
        console.log(`🎉 DONE — ${config.label} synced at ${new Date().toLocaleString()}`);
        console.log(`${'═'.repeat(60)}\n`);
