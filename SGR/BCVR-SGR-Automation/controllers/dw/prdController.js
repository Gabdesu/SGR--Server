...
async function getSpreadsheetIdForCurrentMonth(folderId, branchCode) {
  ...
  console.log(
    `🔎 [PRD] Searching Drive folder for: "${currentFileName}" (Branch: ${branchCode})`,
  );
...
const queries = {
  ...
  "Fast Moving (Meds)": `
    ...
    SELECT TOP 100
        CASE 
            WHEN MAX(TBL_Category_File.group_ID) = 1 AND LEN(MAX(I.Item_Name)) > 4
            THEN LEFT(MAX(I.Item_Name), LEN(MAX(I.Item_Name)) - 4)
            ELSE MAX(I.Item_Name) 
        END AS Product,
    ...
    WHERE TBL_Category_File.group_ID = 1 -- Group 1: Medicines
    ...
    GROUP BY CASE 
        WHEN LEN(I.Item_Name) > 4 THEN LEFT(I.Item_Name, LEN(I.Item_Name) - 4) 
        ELSE I.Item_Name 
    END
  ...
  "Fast Moving (Supplies)": `
    ...
    SELECT TOP 100
        CASE 
            WHEN MAX(TBL_Category_File.group_ID) = 1 AND LEN(MAX(I.Item_Name)) > 4
            THEN LEFT(MAX(I.6Item_Name), LEN(MAX(I.Item_Name)) - 4)
            ELSE MAX(I.Item_Name) 
        END AS Product,
        COUNT(O.order_no) AS "Freq.",
        SUM(CASE WHEN order_type LIKE '%Sales%' AND order_type NOT LIKE '%Transfer%' THEN QTY ELSE 0 END) AS Quantity,
    ...
    INNER JOIN TBL_Category_Item_File I ON I.Item_ID = OD.Item_ID
    INNER JOIN TBL_Category_File ON TBL_Category_File.Catg_ID = I.Catg_ID
    WHERE TBL_Category_File.group_ID = 2 -- Group 2: Supplies
      AND Order_Type LIKE '%Sales%'
      AND (CASE 
            WHEN Encode_DateTime IS NULL OR Encode_DateTime = '' THEN CONVERT(DATE, Order_Date)
            ELSE CONVERT(DATE, Encode_DateTime) 
  ...
  Procurement: `
    ...
    INNER JOIN TBL_Orders_Lacking ON TBL_Orders_Lacking.OL_ID = TBL_Orders_Lacking_Details.OL_ID
    WHERE Item_Status NOT IN ('Served', 'Changed Item', 'Waived')
  ...
  "Procurement-Special": `
    ...
    FROM TBL_Orders_Lacking_Details
    INNER JOIN TBL_Orders_Lacking ON TBL_Orders_Lacking.OL_ID = TBL_Orders_Lacking_Details.OL_ID
    WHERE Item_Status IN ('Changed Item', 'Waived')
      AND OL_BookingDate <= @EndDate
    ORDER BY Item_Description;`,
...
const PESO_KEYS =
  /price|total|value|sold|amount|cost|disc|orig|capital|distribution/i;
  ...
  try {
    ...
    if (sheetName === "High Peso Value") {
      ...
      if (rows && rows.length > 0) {
        ...
        if (meds.length > 0)
          batchData.push({
            ...
            values: [hpvHeaders, ...meds],
          });
        if (supplies.length > 0)
          batchData.push({
            range: `'${sheetName}'!H${START_ROW}`,
            values: [hpvHeaders, ...supplies],
          });
        ...
        console.log(
          `✅ [${sheetName}] Side-by-side sync completed (headers on Row ${START_ROW}).`,
        );
      } else {
        ...
        console.log(
          `⚠️ [${sheetName}] No records — headers written on Row ${START_ROW}.`,
        );
      }
      ...
      if (rows && rows.length > 0) {
        ...
        console.log(
          `✅ [${sheetName}] Data synced (headers on Row ${START_ROW}, data from Row ${START_ROW + 1}).`,
        );
      } else {
...
exports.run = async (pool, folderId, branchCode) => {
  ...
  for (let i = 0; i < tabNames.length; i++) {
    ...
    console.log(
      `   ⏳ [PRD] [${i + 1}/${tabNames.length}] Syncing: "${tabName}"...`,
    );
