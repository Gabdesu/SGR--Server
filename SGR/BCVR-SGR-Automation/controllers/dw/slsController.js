...
const { slsQueries } = require("../../Model/DwQueries");
  ...
  try {
    ...
    if (sheetName === "Top 30") {
      ...
      if (t1Headers.length) {
        ...
        if (!rows.length)
          console.warn(`⚠️  [${sheetName}] Table 1 (Booking): 0 rows. Header written.`);
      }
      ...
      if (t2Headers.length) {
        ...
        if (!rows.length)
          console.warn(`⚠️  [${sheetName}] Table 2 (Delivered): 0 rows. Header written.`);
      }
      ...
      if (t3Headers.length) {
        ...
        if (!rows.length)
          console.warn(`⚠️  [${sheetName}] Table 3 (Collected): 0 rows. Header written.`);
      }
